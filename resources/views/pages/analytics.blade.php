...
@section('headerTitle', 'Analytics Dashboard')

@section('content')
<div class="flex justify-between items-center mb-6">
    <div class="flex space-x-2">
        <x-ui.button variant="primary" data-action="export-data">
            Export Data
        </x-ui.button>
        <x-ui.button variant="outline" data-action="schedule-report">
            Schedule Report
        </x-ui.button>
    </div>
</div>
                ...
                @forelse($topProducts ?? [] as $product)
                    <div>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full progress-bar" data-width="{{ $product->percentage }}"></div>
                        </div>
                    </div>
                @empty
            ...
            <div class="space-y-4">
                @forelse($customerSegments ?? [] as $segment)
                    <div>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full progress-bar" data-width="{{ $segment->percentage }}"></div>
                        </div>
                    </div>
                @empty
...
@push('scripts')
<script>
    // Define chart data variables within a global object to avoid conflicts
    window.analyticsData = {
        revenueLabels: {!! json_encode($revenueTrendsData['labels'] ?? []) !!},
        revenueData: {!! json_encode($revenueTrendsData['data'] ?? []) !!}
    };
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Chart.js for Revenue Trends
        const ctx = document.getElementById('revenueTrendsChart');
        if (ctx) {
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: window.analyticsData.revenueLabels,
                    datasets: [{
                        label: 'Revenue',
                        data: window.analyticsData.revenueData,
                        borderColor: '#4299e1',
        ...
        document.querySelector('button[data-action="export-data"]')?.addEventListener('click', function() {
            alert('Export Data button clicked!');
        });

        document.querySelector('button[data-action="schedule-report"]')?.addEventListener('click', function() {
            alert('Schedule Report button clicked!');
        });
    });
</script>
@endpush