...
<div class="content-card chat-page" style="height: 100%; display: flex; flex-direction: column; padding: 0;">
  <div class="chat-flex-container" style="display: flex; flex: 1; height: 100%; overflow: hidden;">
    ...
    <!-- Main Chat Area -->
    <div class="chat-main">
      <div id="chat-header-area" style="display:none; align-items:center; gap:16px; padding: 1.2rem 2rem 0.5rem 2rem; border-bottom: 1.5px solid #e5e7eb; background: #fff;">
      ...
      <div id="chat-messages-area" class="chat-messages" style="height: 70vh; overflow-y: auto; padding: 2rem 2rem 1rem 2rem; background: #f8fafc; border-radius: 0 0 18px 18px;">
        ...
        <div style="text-align: left; display: inline-block; margin-bottom: 2.2rem;">
          ...
          <div style="color: #166534; font-size: 1.1rem; margin-bottom: 0.7rem;"><i class="fas fa-paper-plane"></i> Send messages, share updates, or assist users in real time.</div>
        ...
        <div style="color: #f97a00; font-size: 1.15rem; margin-top: 2.2rem; text-align: center;">Need help? Use the chat to support your users efficiently!</div>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.user-chat-link').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            fetch(`/admin/chat/messages/${selectedUserId}`)
                .then(res => res.json())
                .then(data => {
                    if (data.status === 'success') {
    ...
    const buttons = document.querySelectorAll('#emoji-btn, #file-upload-btn, #send-btn');
    buttons.forEach(btn => {
        btn.addEventListener('mouseenter', function() {
            this.style.color = this.id === 'send-btn' ? '#1e40af' : '#666';
        });
        btn.addEventListener('mouseleave', function() {
            this.style.color = this.id === 'send-btn' ? 'white' : '#888';
        });
    });
});
</script>
